  ...
  <table width="100%" rules="all" border="1">
    ...
    <?php
      ...
      $q = mysqli_query($link,"SELECT 
            ...
            ORDER BY stok.id_toko, produk.kd_produk ASC
            ") or die (mysqli_error());
    ...
    ?>
        <tr>
          ...
          while ($size = mysqli_fetch_array($sizes)) {
            $stokToko = mysqli_fetch_array(mysqli_query($link,"SELECT
              stok.jumlah
              FROM 
              stok
              INNER JOIN produk_size ON produk_size.kd_produk_size = stok.kd_produk_size
              $qWhere 
              $qToko
              AND produk_size.id_size = '$size[id_size]'
              AND produk_size.kd_produk = '$r[kd_produk]'
              "));
            $jumlah = $stokToko['jumlah'];
            if ($stokToko == null) { 
              $jumlah = 0;
            }
            $total += $jumlah;
