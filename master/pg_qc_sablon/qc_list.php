      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-body table-responsive">
            <a href="?act=<?php echo md5('qc_sablon_add')?>" class="btn btn-info"><i class="fa fa-plus"></i> Tambah Data</a>
            <hr>
              ...
              <div class="row">
                <div class="col-md-3">
                  <select class="form-control select2" name="id_cmt" id="id_cmt">
                    <option value="">Semua Sablon</option>
                    <?php
            ...
            <table class="table">
              ...
              <?php
                ...
                if ($_GET['id_vendor'] != '') {
                  $where .= "qc.id_vendor = '$_GET[id_vendor]' ";
                } else {
                  $where .= "qc.id_vendor != '' ";
                }
                ...
                $q = mysqli_query($link,"SELECT 
                  *   
                  FROM 
                  qc 
                  INNER JOIN vendor ON vendor.id_vendor = qc.id_vendor
                  $where 
                  ORDER BY id_qc DESC 
                  LIMIT $limit
                  ");
                if (mysqli_num_rows($q) > 0) {
                  while ($r = mysqli_fetch_array($q)) {
                    ...
                    $status = "<a href='?act=".md5('qc_sablon_approve')."&id_qc=$id_qc&lns=$satu' title='Ubah Status' onclick='return confirm(\"Apakah anda yakin?\")'><span class='label label-warning'>Pending</span></a>";
                    if ($r['status'] == '1') {
                      $status = "<a href='?act=".md5('qc_sablon_approve')."&id_qc=$id_qc&lns=$nol' title='Ubah Status' onclick='return confirm(\"Apakah anda yakin?\")'><span class='label label-success'>Done</span></a>";
                    }
              ?>
                    <tr>
                      <td><?= $i;?></td>
                      <td>
                        <div class="btn-group">
                          ...
                          <ul class="dropdown-menu" role="menu">
                            ...
                            ?>
                              <li><a href="?act=<?php echo md5('qc_sablon_edit')."&id_qc=$id_qc"?>"><i class="fa fa-pencil"></i>Ubah</a></li>
                              <?php
                              if ($_SESSION['id_level'] == 1) {
                              ?>
                                <li><a href="?act=<?php echo md5('qc_sablon_delete')."&id_qc=$id_qc"?>" onclick="return confirm('Apakah anda yakin ?')"><i class="fa fa-trash"></i>Hapus</a></li>
                              <?php
                              }
                              ?>
                            <?php
